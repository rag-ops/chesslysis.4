...
export default function PhasePerformance({ rows }: { rows: Row[] }) {
  return <section className="rounded-xl border border-white/10 bg-[#0d1320] p-5 shadow-2xl shadow-black/20">
    <h2 className="font-semibold text-white">Performance by game phase</h2>
    <div className="mt-4 grid gap-3 md:grid-cols-3">
      {rows.map((row) => <div key={row.phase} className="rounded-lg border border-white/10 bg-white/[.03] p-4">
        <div className="capitalize font-medium text-white">{row.phase}</div>

...
export function openingLabel(opening: string | null, eco: string | null) {
  ...
  const humanize = (value: string) => {
    try {
      ...
      if (slug) {
        ...
        // Chess.com commonly encodes possessives as `Queen-s-Gambit` in slugs.
        // Title-case first, then restore the possessive with a lowercase `s`.
        return words
  ...
  if (eco?.trim()) {
    const candidate = eco.trim();
    if (/^https?:\/\//i.test(candidate)) return humanize(candidate);
    return /^ECO\s/i.test(candidate) ? candidate : `ECO ${candidate}`;
  }
...
export async function getPlayerOpeningIntelligence(username: string) {
  ...
  for (const game of games) {
    ...
    if (game.analysisStatus === "COMPLETED" && game.statistics) {
      const accuracy = safeAccuracy(isWhite ? game.statistics.whiteAccuracy : game.statistics.blackAccuracy);
      const acpl = safeACPL(isWhite ? game.statistics.whiteACPL : game.statistics.blackACPL);

...
export async function getPlayerDashboard(username: string) {
  ...
  return {
    ...
    stats: {
      ...
      averageAccuracy: accuracy.length ? round(mean(accuracy)) : null,
      averageACPL: acpl.length ? round(mean(acpl)) : null,
      blundersPerGame: blunders.length ? round(mean(blunders)) : null,
    },
...
function playerAccuracy(g: GameRow,u:string):number|null { return g.statistics ? safeAccuracy(g.whiteUsername.toLowerCase()===u.toLowerCase()?g.statistics.whiteAccuracy:g.statistics.blackAccuracy) : null; }
function playerACPL(g: GameRow,u:string):number|null { return g.statistics ? safeACPL(g.whiteUsername.toLowerCase()===u.toLowerCase()?g.statistics.whiteACPL:g.statistics.blackACPL) : null; }

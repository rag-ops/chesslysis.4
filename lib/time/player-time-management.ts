...
import { safeACPL, safeAccuracy } from "@/lib/analysis/metrics";
...
export function parseTimeControl(value?: string | null): { bucket: TimeBucket; label: string; baseSeconds: number | null; increment: number | null } {
  ...
  // Chess.com stores most controls as base seconds + optional increment seconds, e.g. 180+2.
  const match = normalized.match(/^(\d+)(?:\+(\d+))?$/);
  if (!match) return { bucket: "unknown", label: value.trim(), baseSeconds: null, increment: null };
  const baseSeconds = Number(match[1]);
  const increment = Number(match[2] ?? 0);
  // Keep categories intuitive and stable: base clock determines the family.
  // Bullet: <=2 min (1, 2, 2+1), Blitz: 3-9 min (3, 3+2, 5), Rapid: >=10 min.
  const bucket: TimeBucket = baseSeconds <= 120 ? "bullet" : baseSeconds < 600 ? "blitz" : "rapid";
  const minutes = baseSeconds / 60;
  const baseLabel = Number.isInteger(minutes) ? `${minutes} min` : `${Math.floor(minutes)}:${String(baseSeconds % 60).padStart(2, "0")}`;
  const label = increment ? `${baseLabel} + ${increment}` : baseLabel;
...
export async function getPlayerTimeManagement(username: string) {
  ...
  const insights: TimeControlInsight[] = [...groups.entries()].map(([bucket, rows]) => {
    ...
    const accuracy = analyzed.map(g => safeAccuracy(g.whiteUsername.toLowerCase() === normalized.toLowerCase() ? g.statistics!.whiteAccuracy : g.statistics!.blackAccuracy)).filter((v): v is number => v !== null);
    const acpl = analyzed.map(g => safeACPL(g.whiteUsername.toLowerCase() === normalized.toLowerCase() ? g.statistics!.whiteACPL : g.statistics!.blackACPL)).filter((v): v is number => v !== null);

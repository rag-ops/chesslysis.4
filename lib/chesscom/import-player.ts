...
export async function importPlayerGames(
  ...
  for (const game of games) {
    ...
    try {
      ...
      const opening = parsed.headers.Opening?.trim() || parsed.headers.ECO?.trim() || game.eco || null;
      const eco = parsed.headers.ECO?.trim() || (game.eco && !/^https?:\/\//i.test(game.eco) ? game.eco : null);

...
import { safeAccuracy, safeEvaluationLoss } from "@/lib/analysis/metrics";
...
export async function getPlayerInsights(username: string) {
  ...
  for (const game of games) {
    ...
    const openingName = openingLabel(game.opening, game.eco);
    ...
    for (const move of game.moves) {
      ...
      bucket.moves++;
      const loss = safeEvaluationLoss(move.engineAnalysis?.evaluationLoss);
      if (loss !== null) bucket.losses.push(loss);

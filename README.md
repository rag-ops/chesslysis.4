...
The container validates `DATABASE_URL` and, by default, applies `prisma db push --skip-generate` before starting. Set `RUN_DB_PUSH_ON_STARTUP=false` only when schema deployment is handled externally. For long-lived production environments, replace `db push` with reviewed Prisma migrations.
...
ANALYSIS_WORKER_TOKEN=<long random secret>
RUN_DB_PUSH_ON_STARTUP=true
NODE_ENV=production
```
...
The visual system follows the supplied editorial Chesslysis reference while preserving the functional Next.js routes, real Chess.com import, PostgreSQL analytics, Stockfish queue and game inspector. The chessboard uses fixed 8×8 geometry and consistent SVG piece assets so squares and pieces remain aligned across viewport sizes.


## Phase 4 integration release

The UI reference is treated as a presentation system, not a replacement for Chesslysis functionality. All username import, multi-game persistence, Stockfish analysis, queue processing, insights, training, openings, time-control analytics, DNA, data health and game inspection routes remain backed by the existing APIs and PostgreSQL data.

The Game Review board uses fixed 8×8 geometry with consistent SVG piece sizing. Exact time controls remain grouped beneath Bullet, Blitz, Rapid and Daily while retaining the original control label (for example 3+2, 5+0, 10+0 and 15+10).

## UI Stabilization

The feature pages now share the same editorial design system as the homepage, with responsive navigation and persistent light/dark theme support. See `UI_STABILIZATION.md` for the regression checklist.

...
function workerHeartbeat() {
  try {
    ...
    return { configured: process.env.ANALYSIS_WORKER_ENABLED !== "false", alive: ageMs < 15_000, ageMs: Number.isFinite(ageMs) ? ageMs : null, ...value };
  } catch { return { configured: process.env.ANALYSIS_WORKER_ENABLED !== "false", alive: false, ageMs: null, state: "no-heartbeat" }; }
}
